...
| **Phase 2 — Parsing** | Resume/doc parsing (OpenRouter + OCR) populates the **same** `Answer.rawValue` (`source: "parsed"`/`"merged"`). Those raw values flow through the **same** rubric functions → same `ParameterValues`. **No engine change, no new `runScore` path** — parsing is upstream of the rubric layer. See [parsing.md](./parsing.md). |
